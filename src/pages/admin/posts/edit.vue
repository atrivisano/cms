    ...
    <div class="py-10">
      <header class="bg-white shadow-sm mb-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between items-center py-6">
            <h1 class="text-3xl font-bold leading-tight text-gray-900">Edit Post</h1>
      ...
      <main>
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div v-else class="bg-white shadow-sm rounded-lg p-8">
...
import UploadModal from '../media/UploadModal.vue'
...
async function loadPost() {
  try {
    ...
    // Handle content from JSON format
    try {
      const parsedContent = JSON.parse(data.content)
      data.content = parsedContent.blocks?.[0]?.content || ''
    } catch (e) {
    ...
    selectedCategories.value = data.posts_categories?.map(pc => pc.category_id) || []
  } catch (e) {
...
const savePost = async (newStatus) => {
  try {
    ...
    if (!session) {
      throw new Error('Please sign in to update the post')
    }

    const postData = {
      ...post.value,
      status: newStatus,
      posts_categories: undefined, // Remove posts_categories from the update
      content: post.value.content, // Store raw markdown content
      published_at: newStatus === 'published' ? (post.value.published_at || new Date().toISOString()) : post.value.published_at
    }
